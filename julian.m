...
%Here is a formula for determining the julian day number:
...
julian_day = [];     %need julian_day to assign an empty matrix for the 
...
for k=1:r
    ...
    elseif m(k)>2
        x=((.4)*m(k) + 2.3);  %necessary value for x if m>2
        yp=y;            %necessary value for yp if m>2
        round x;
    end   %end statement of "if m(k)<=2"
...
end   %end statement of "for k=1:r"
